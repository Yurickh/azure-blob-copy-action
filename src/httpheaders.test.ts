...
test("resolveContentType", () => {
  ...
  expect(resolveContentType("test.js")).toBe(
    "application/javascript; charset=utf-8",
  )
  expect(resolveContentType("test.js.map")).toBe(
    "application/json; charset=utf-8",
  )
  expect(resolveContentType("test.json")).toBe(
    "application/json; charset=utf-8",
  )
...
test("resolveHttpHeaders", () => {
  const resolve = function (httpHeaders?: HttpHeadersOptions) {
    return resolveHttpHeaders("folder1/test.js", {
      localDirectory: ".",
      httpHeaders,
    })
  }
  ...
  expect(resolve([]).blobCacheControl).toBeUndefined()
  expect(
    resolve([{ glob: "**/*.ts", httpHeaders: { blobCacheControl: "public" } }])
      .blobCacheControl,
  ).toBeUndefined()
  expect(
    resolve([{ glob: "**/*.js", httpHeaders: { blobCacheControl: "public" } }])
      .blobCacheControl,
  ).toBe("public")
})
